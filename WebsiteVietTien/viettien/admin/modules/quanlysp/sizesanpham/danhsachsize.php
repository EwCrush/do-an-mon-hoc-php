<?php 
    ...
    $trangtiep = 0;
    $trangtruoc = 0;
    $sql_danhsach_dssize_limit = "SELECT * FROM size where MaSP = $id";
    ...
    //     $tonghang++;
    // }
    $stmt_limit = $conn->prepare($sql_danhsach_dssize_limit);
    $stmt_limit->execute();
                        ...
                        <div class="content">
                            <div class="heading-content">
                                <div class="heading-content-back">
                                    ...
                                    <div class="heading-content-text">Thông tin về size của sản phẩm <?php echo $tensp?></div>
                                </div>
                            </div>
                            <?php
                                include("./config/config.php");
                                $sql_danhsach_chitiet_sp = "SELECT * from size where masp = $id LIMIT 10 OFFSET $offset";
                            ...
                            <table class="table-content">
                                <tr class="table-row-content">
                                    <th class="row-40 table-heading-content">Size sản phẩm</th>
                                    <th class="row-40 table-heading-content">Số lượng</th>
                                ...
                                ?>
                                <tr class="table-row-content">
                                    <td class="row-40 table_info"><?php echo $row['SizeSP'] ?></td>
                                    <td class="row-40 table_info"><?php echo $row['SoLuong'] ?></td>
                                    <td class="row-20 table_info">
                                        <a href='index.php?action=suasize&size=<?php echo $row["SizeSP"]?>&id=<?php echo $row["MaSP"]?>' class='tb_thaotac_link'>
                                            <i class='tb_thaotac_link_icon fa-sharp fa-solid fa-pen' title='Sửa'></i>
                                        </a>
                                        <a href='index.php?action=xoasize&size=<?php echo $row["SizeSP"]?>&id=<?php echo $row["MaSP"]?>' class='btn-delete tb_thaotac_link'>
                            ...
                            <div class="pagination-wrapper">
                                <div class="pagination-other-features">
                                    <a href="index.php?action=themsize&id=<?php echo $id?>" class="ThemMoi">
                                        <i class="pagination-item fa-solid fa-circle-plus" title="Thêm"></i>
                                    </a>
                                </div>
                                <div class="pagination">
                                    <a href="index.php?action=sizesanpham&trang=<?php echo $trangtruoc?>&id=<?php echo $id?>" class="ThemMoi">
                                    ...
                                    <a href="index.php?action=sizesanpham&trang=<?php echo $trangtiep ?>&id=<?php echo $id?>" class="ThemMoi">
